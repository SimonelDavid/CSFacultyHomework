...
void _test_undo() {
    ...
    try {
        ...
        assert(e.getMessage() == "No more undo's to perform.");
    }

    code = test_subject.addElem(gen.genString(), "100", gen.genString(), gen.genString());
    codes.push_back(code);
    code = test_subject.addElem(gen.genString(), "100", gen.genString(), gen.genString());
    codes.push_back(code);
    
    test_subject.deleteElem(codes[1]);

    entity aux(gen.genString(), 1000, gen.genString(), gen.genString());
    test_subject.changeElem(codes[1], aux.getName(), "1000", aux.getManufacturer(), gen.genString());
    assert(test_subject.getElem(codes[1]) == aux);
