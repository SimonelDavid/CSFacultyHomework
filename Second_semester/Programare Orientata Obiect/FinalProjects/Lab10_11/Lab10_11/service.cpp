...
void service_reteta::genereazaReteta(int nrTotal){
    if(serv.getNrElems() == 0)
        throw RangeError("Nu sunt elemente in serv.");
    vector<entity> aux;
    iter_pair copy = serv.getAll();
    for(;copy.first!=copy.second;++copy.first)
            aux.push_back(*(copy.first));
...
int service::getFilterNrElems(){
    return filter.getNrElems();
}
