	...
	QObject::connect(butonAdauga, &QPushButton::clicked, [&]()
		{
			try
			{
				...
				loadDataReteta(s.getReteta(), "#51ddfc");
			}
	...
	QObject::connect(butonGenereaza, &QPushButton::clicked, [&]()
		{
			...
			loadDataReteta(s.getReteta(), "#51ddfc");
						  ...
						  //&QListWidget
	QObject::connect(lst1, &QTableWidget::itemSelectionChanged, [&]()
		{
			auto sel = lst1->selectedItems();
			if (sel.isEmpty())
			{
...
{
	lst1->clearContents();
	lst1->setRowCount(s.getNrMeds());
	lst1->setColumnCount(4);
	...
	for (const auto& med : meds)
	{
		...
		lst1->insertRow(line_number);
		lst1->setItem(line_number, 0, new QTableWidgetItem(QString::fromStdString(med.getDenumire())));
		lst1->setItem(line_number, 1, new QTableWidgetItem(QString::fromStdString(med.getProducator())));
		lst1->setItem(line_number, 2, new QTableWidgetItem(QString::fromStdString(to_string(med.getPret()))));
		lst1->setItem(line_number, 3, new QTableWidgetItem(QString::fromStdString(to_string(med.getSubstantaActiva()))));
