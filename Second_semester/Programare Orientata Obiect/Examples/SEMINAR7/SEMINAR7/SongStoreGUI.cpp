...
void SongStoreGUI::connectSignalsSlots() {
	...
	QObject::connect(tableSongs, &QTableWidget::itemSelectionChanged, [&]() {
		QList<QTableWidgetItem*> itemList;
		for (int row = 0; row < tableSongs->rowCount(); ++row) {
			for (int col = 0; col < tableSongs->columnCount(); ++col) {
				itemList << tableSongs->item(row, col);
			}
		}
		...
		int row = tableSongs->selectedRanges().constFirst().topRow();
		for (int j = 0; j < this->tableSongs->columnCount(); j++)
			tableSongs->item(row, j)->setBackground(Qt::magenta); });
