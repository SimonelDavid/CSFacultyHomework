...
Pet PetService::find(const string& type, const string& species) {
	return repo.find(type, species);
}
/*
Adauga pet
*/
void PetService::add(const string& type, const string& species, int price) {
...
Toate
*/
const std::vector<Pet>& PetService::getAllPets()const {
	return repo.getAll();	
}
...
Sortat dupa pret
*/
std::vector<Pet> PetService::getSortByPrice()const {
	auto v = repo.getAll();
...
Sortat dupa tip
*/
std::vector<Pet> PetService::getSortByType() const {
	auto v = repo.getAll();
...
Filtrare dupa pret
*/
std::vector<Pet> PetService::getFilterByPrice(int price) const {
	std::vector<Pet> rez;
...
const std::vector<Pet>& PetService::addToCos(const std::string& type,const std::string& species) {
	const Pet& p = repo.find(type,species);
	cos.adauga(p);
	return cos.lista();
}

const std::vector<Pet>& PetService::addRandom(int cate) {
	cos.umple(cate,repo.getAll());
	return cos.lista();
}

const std::vector<Pet>& PetService::golesteCos() {
	cos.goleste();
	return cos.lista();
}

const std::vector<Pet>& PetService::toateDinCos() {
	return cos.lista();
}

void PetService::exportaCosCVS(std::string fName) const {
	exportToCVS(fName,cos.lista());
}

void PetService::exportaCosHTML(std::string fName) const {
