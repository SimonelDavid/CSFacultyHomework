...
vector<DTO> Service::makeDTOVector() const
{
	vector<Medicament> meds = this->getMeds();
	vector<DTO> dto;
	for (const auto& med : meds)
	{
		...
		for (auto& subst : dto)
		{
			if (subst.getSubstantaActiva() == med.getSubstantaActiva())
			{
				gasit = true;
				subst.setNrMedicamente(subst.getNrMedicamente() + 1);
			}
		}
		if (gasit == false)
		{
			DTO d{ med.getSubstantaActiva(), 1 };
			dto.push_back(d);
		}
	}
	return dto;
...
Medicament Service::findMedReteta(const string& denumire)
{
	Medicament gasit = ret.findMed(denumire);
	return gasit;
}
