package examen.network.rpcprotocol;
...
public class ServicesRpcProxy implements IService {
    ...
    public void logout(Jurry game, IObserver iObserver) throws MyException {
...
//            try {
//                Jurry jurry = (Jurry) response.data();
//                employeeObserver.updateControl(jurry);
//            } catch (MyException e) {
//                logger.error("Error handle update: " + e);
//            }
//        }
//    }
    ...
    @Override
    public Jurry login(Jurry jurry, IObserver iObserver) throws MyException {
        initializeConnection();
        Request req = new Request.Builder().type(RequestType.LOGIN).data(jurry).build();
        sendRequest(req);
        Response response = readResponse();
        if (response.type() == ResponseType.OK) {
            this.employeeObserver = iObserver;
            logger.info("Logged in");
            return (Jurry) response.data();
        }
        if (response.type() == ResponseType.ERROR) {
...
//    @Override
//    public String setControlGameOver(Jurry jurry) throws MyException {
//        Request req = new Request.Builder().type(RequestType.SET_CONTROL_GAME_OVER).data(jurry).build();
//        sendRequest(req);
//        Response response = readResponse();
//        if (response.type() == ResponseType.ERROR) {
